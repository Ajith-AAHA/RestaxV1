...
let DepartmentDataComponent = class DepartmentDataComponent {
    constructor(fb) {
        this.fb = fb;
...
let FacultydockComponent = class FacultydockComponent {
    ...
    createfaculty(facultyname, facultyemail, pwt, pwi, pwc) {
        this.apollo
            ...
            update: (proxy, { data: { addfaculty } }) => {
                ...
                const data = proxy.readQuery({ query: _query__WEBPACK_IMPORTED_MODULE_4__["faculties"] });
                data.faculties.push(addfaculty);
                // write our data back to cache
                proxy.writeQuery({ query: _query__WEBPACK_IMPORTED_MODULE_4__["faculties"], data });
            },
        }).subscribe(({ data }) => {
    ...
    getAllusers() {
        this.apollo.watchQuery({ query: _query__WEBPACK_IMPORTED_MODULE_4__["faculties"] })
            .valueChanges
            .map((result) => result.data.faculties).subscribe((data) => {
            this.faculties = data;
        });
    }
    /**
...
const addfaculty = graphql_tag__WEBPACK_IMPORTED_MODULE_0___default.a `
  mutation addfaculty(
    $facultyname: String!,
    $facultyemail: String!,
    $pwt: Boolean!,
...
`;
const faculties = graphql_tag__WEBPACK_IMPORTED_MODULE_0___default.a `
  query {
    faculties{
      id
      facultyname
...
const removefaculty = graphql_tag__WEBPACK_IMPORTED_MODULE_0___default.a `
...
const updatefaculty = graphql_tag__WEBPACK_IMPORTED_MODULE_0___default.a `
